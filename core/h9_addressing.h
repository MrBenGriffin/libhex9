...
static void h9_bin_uuid(const uint8_t in[16], int layer, uint8_t out[16]) {
     ...
     *
     * FOSSIL (docs/addressing-doctrine.md, F3): bin→coarser re-binning is
    ...
    if (nibbles[H9_NIB_TAIL - 1] == 0x0Fu) {
        ...
        top = bl;
        if (layer >= top) {
            /* Re-binning a bin at its own layer is the identity (bins are
             * already canonical); deeper layers are unreachable — the
             * sub-cell digits are gone. Return the input unchanged; callers
             * that must reject deepening validate `layer` themselves. */
            memcpy(out, in, 16);
            return;
        }
        /* Bin key_tail stores (c2 << 1) | r_mo — no c_mo bit (Python
         * tail_pack_key format). Recover c_mo the way h9_cell's unpack
         * does: try both values and keep the chain that terminates at the
         * expected L0 c2 (fall back to 1 when neither matches). */
        const uint8_t expected_c2 = H9_L0HEX_BACK[nibbles[0]][r_mo][1];
        c_mo = 1u;
        for (uint8_t try_cmo = 0; try_cmo < 2; ++try_cmo) {
            uint8_t tc_mo = try_cmo, tc2 = c2;
            for (int l = top; l >= 1; --l) {
                const uint8_t *e = H9_HEX_REG[nibbles[l]][tc_mo][tc2];
                tc_mo = e[1]; tc2 = e[2];
            }
            if (tc2 == expected_c2) { c_mo = try_cmo; break; }
        }
    }
