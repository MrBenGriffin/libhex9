...
struct H9GridCell {
    uint8_t uuid[16];      /* bin UUID at the cell's layer (grouping key) */
    uint8_t id_uuid[16];   /* full L29 identity UUID (reversible address) */
...
/* (lon, lat) degrees → the WARPED octant coordinate (b_oct), i.e. the same
 * (cx, cy) the descent operates in: BRAW from lonlatdeg_to_cxcy_oid, then
 * h9_warp_inv (== AuthalicWarp.undo) — the authalic warp Python applies in
 * b_raw → b_oct. NO descent preamble nudge (that is descent-internal, not part
 * of the b_oct coordinate). Respects the runtime warp toggle: with the warp
...
/* Inverse of lonlatdeg_to_boct_oid: WARPED octant coordinate (b_oct) → (lon,lat)
 * degrees. cxcy_to_lonlat already IS the full inverse — it un-warps with
 * h9_warp_fwd (== AuthalicWarp.do, b_oct → BRAW) then runs the geometric
 * BRAW → c_oct → lon/lat — and honours the same warp toggle (off ⇒ input is
 * treated as BRAW), so project/unproject are exact inverses in both modes
 ...
 *   descending it is classification-unstable and bins back wrong (~22% of
 *   cells; worse — the beam encoder shears at seam cells at fine layers). We
 *   instead descend an interior point of the canonical MODE-0 half: nudge the
 *   centroid a fixed fraction toward the mode-0 supercell origin (off the
 *   seam), warp-forward into the b_oct frame, and run the grid-canonical
 *   `uuid_from_cxcy` descent to L29. Validated h9_bin(id,L)==bin and unique on
 *   ~16k cells spanning mid-latitude, the lon=0 octant seam, and a near-pole
 *   box, at L8/11/13/16/20.
 *
 * `cen_cx,cen_cy` is the cell's mode-matching centroid in its oid b_oct frame
 * (as emit_hexes_for_oid computes it); `(ia,ib)` the mode-0 supercell origin
 * in integer UV at `layer`. decode(id) lands ~10% of a circumradius off the
 * centroid — use the geometric centroid for display, this UUID for the key. */
static inline void full_id_from_cell(double cen_cx, double cen_cy,
                                     int64_t ia, int64_t ib, int oid, int layer,
                                     uint8_t out[16]) {
    const double NUDGE  = 0.10;   /* interior fraction toward the mode-0 corner */
    const double div_f  = std::pow(3.0, (double)layer);
    const double org_cx = (double)ia * H9_UV_U1 / div_f;
                                      ...
                                      std::vector<H9GridCell> &out)
{
    ...
    for (size_t qi = 0; qi < q_cur.size(); qi++) {
        ...
        for (int c2 = 0; c2 < H9_N_C2; c2++) {
            ...
            /* Full L29 identity UUID: interior point of the mode-0 half,
             * descended encoder-free (see full_id_from_cell). The centroid
             * (cen_cx, cen_cy) is reused directly. */
            uint8_t id_uuid[16];
            full_id_from_cell(cen_cx, cen_cy, ia_org, ib_org, oid, layer, id_uuid);

            H9GridCell cell;
            std::memcpy(cell.uuid,    bin_uuid, 16);
            std::memcpy(cell.id_uuid, id_uuid,  16);
            cell.oid     = oid;
            cell.c2      = c2;
            cell.ia      = ia_org;
            cell.ib      = ib_org;
