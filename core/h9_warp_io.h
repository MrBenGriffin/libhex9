...
struct H9WarpData {
    ...
    /* v3 (F6 edge-tangent field): the FINAL per-vertex CT gradients
     * computed by hhg9 (ghost-orbit padding + x-mirror symmetrisation +
     * edge-tangent projection already applied). grad_dx[i] = ∇(dx) at
     * tri_mesh vertex i, grad_dy[i] = ∇(dy). Empty for v1/v2 fields —
                        ...
                        H9WarpData& out, std::string* err = nullptr)
{
    ...
    if (out.header.version >= 3 && (out.header.flags & H9W_FLAG_HAS_GRADIENTS)) {
        ...
        if (out.header.dtype_code != H9W_F64) {
            if (err) *err = "h9warp v3: gradients require f64 payload";
            return false;
        }
        if (payload_len != static_cast<std::size_t>(out.header.count) * 48) {
        ...
        out.grad_dy.resize(out.header.count);
        for (std::uint32_t i = 0; i < out.header.count; ++i) {
            const std::uint8_t* p = payload + static_cast<std::size_t>(i) * 48;
            out.deltas[i][0]  = detail::read_le<double>(p, 0);
            out.deltas[i][1]  = detail::read_le<double>(p, 8);
            out.grad_dx[i][0] = detail::read_le<double>(p, 16);
            out.grad_dx[i][1] = detail::read_le<double>(p, 24);
            out.grad_dy[i][0] = detail::read_le<double>(p, 32);
            out.grad_dy[i][1] = detail::read_le<double>(p, 40);
        }
        return true;
    }
